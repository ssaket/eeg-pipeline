...
class PipelineTestCase(unittest.TestCase):
    ...
    def test_erp_peaks(self):
        self.pipeline.make_pipeline([SimpleMNEFilter(0.5, 50, 'firwin'), CleaningData(self.bids_path), PrecomputedICA(self.bids_path)])
        erp_clean = ERPAnalysis(-0.1, 0.8)
...
class DecodingAnalysisTestCase(unittest.TestCase):
    ...
    def test_sliding_window(self):
        decoder = EEGDecoder('stimulus', (-0.2, 1), (0.0, 0.8), self.pipeline.raw, baseline=(None, 0), reject_by_annotation=True)
